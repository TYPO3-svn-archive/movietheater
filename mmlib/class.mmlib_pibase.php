...
class mmlib_pibase extends tslib_pibase {
  ...
  public function main($content,$conf,$nocache=0){
    ...
    $this->pi_initPIflexForm();
		...
		if(file_exists( $llf = $this->file($this->conf['locallang']) )){
			$this->LOCAL_LANG = array_merge($this->LOCAL_LANG,t3lib_div::readLLfile($llf,$this->LLkey));
		}
  ...
  private function getLL($key=''){
    ...
    foreach( $this->LOCAL_LANG[$LLkey] as $k => $v) $result[strtoupper('LLL_'.$k)] = $v;
  ...
  */
  public function marker_parse($tpl,$marker){
	...
	public function link($params,$urlParameters=array(),$target=''){
		if(empty($params))$params = $GLOBALS['TSFE']->id;
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/movietheater/mmlib/class.mmlib_pibase.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/movietheater/mmlib/class.mmlib_pibase.php']);
}
?>